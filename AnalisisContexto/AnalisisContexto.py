...
def validateArit(variableTable,expression,line,column):
    ...
    if type(expression) is ExpresionParentizada:
    ...
    if (type(expression) is ExpresionAritmetica):
        ...
        symbol = variableTable.buscarSimbolo(tokenMe)
        if symbol:
            valid = True
        else:
            ...
            return False
    elif variableTable.buscarSimbolo(expression):
        symbol = variableTable.buscarSimbolo(expression)
        if symbol.symbolType == intTipo:
            ...
            print(errorConflictoTipos + expression + notInt + lineInfo
                  + str(line) + columnInfo + str(column) + closeInfo)
            return False                
    elif not variableTable.buscarSimbolo(expression):
...
def AnalisisDeExpresion(variableTable,expression,line,column):
    ...
    # Caso 2: Expresion Relacional    
        ...
        if (valid1):            
            ...
            symbol1 = variableTable.buscarSimbolo(expression.expresion1)
            symbol2 = variableTable.buscarSimbolo(expression.expresion2)
            ...
            if valid2 and (expression.operador == "=" or expression.operador == "/="):
                ...
                # de tipo entero
                elif ((expreType1 is ExpresionAritmetica and symbol2.obtenerTipo() != intTipo)
                    or
                    (expreType1 is ExpresionBooleana and symbol2.obtenerTipo() == intTipo)
                    or
                    (expreType1 is ExpresionRelacional and symbol2.obtenerTipo() == intTipo)):
                    print()
                    return False
                # Una expresion aritmetica solo puede ser igualada a una variable si esta es
                # de tipo entero 
                elif ((expreType2 is ExpresionAritmetica and symbol1.obtenerTipo() != intTipo)
                    or
                    (expreType2 is ExpresionBooleana and symbol1.obtenerTipo() == intTipo)
                    or
                    (expreType2 is ExpresionRelacional and symbol1.obtenerTipo() == intTipo)):
                    print()
                    return False
    ...
    # Caso 3: Expresion Booleana
        ...
        if (valid1):
            ...
            return False   
    elif expressionType is ExpresionParentizada:
        return AnalisisDeExpresion(variableTable,expression.expresion,line,column)
